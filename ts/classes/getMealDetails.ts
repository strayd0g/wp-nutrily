...
import { 
    ...
    meal_temp_data,
    total_time_data,
    ingredients_type_data,
    mean_id,
    all_details_data,
    data_loading,
    } from "../classes/constants.js";

interface I_IngredientsElements {
    name: string,
    quantity: number,
}
...
const createIngredientsElements = (element: I_IngredientsElements) : void => {
...
const createIngredientsElementsOptional = (element: I_IngredientsElements) : void => {
...
export async function getMealDetails(meal_id: number) {

    clearAllFields();

    // fetch meal details based in  meal_id
    await fetch(`/wp-json/nutrily/v1/user/meals/detail/${meal_id}`)
        ...
        .then((data) => {
            ...
            setTimeout(() => {
                ...
                all_details_data?.classList.remove("hidden");
                ...
                let quantity_list_arr = quantity_list?.split(';');
                ...
                ingredients_list_arr.forEach((ingredient_name: any, index: number) => {
                    parse_ingredient_quantity_list.push({['name']: ingredient_name, ['quantity']: quantity_list_arr[index]})
                });

                ingredients_list_optional_arr.forEach((ingredient_name: any, index: number) => {
                ...
                let steps_list_arr = steps_list?.split(';');
                ...
                let image_links_arr = image_links?.split(';');
